...
defineProps<novelDetails>()
const addToLibrary = () => { };
const breakpoints = useBreakpoints(breakpointsTailwind)
</script>
<template>
    <article v-if="breakpoints.smallerOrEqual('md') && useScreenOrientation().orientation.value === 'portrait-primary'"
    ...
    <article v-else
        class="inline-flex snap-always snap-center w-max min-w-[50%] basis-[200px] flex-auto gap-3 max-w-[900px] justify-between rounded-xl border bg-secundary-50 border-secundary-300 dark:bg-neutral-g800 dark:border-neutral-g500 overflow-hidden hover:active:border-secundary-400 dark:hover:active:border-neutral-g200 transition-colors duration-200 ease-in">
        <NuxtImg class="aspect-portrait size-full object-cover max-h-[200px] lg:max-h-[250px] max-w-1/2" :src="image"
            placeholder format="avif" />
        <div class="flex flex-col justify-between gap-3 w-full p-3">
            <div class="flex self-stretch flex-col gap-3 w-full">
                <hgroup class="self-stretch">
                    <h4 @click="navigateTo(novelUrl)" class="line-clamp-2 text-md lg:text-lg font-bold capitalize select-all">
                        {{ title }}</h4>
                    <p
                        class="line-clamp-3 mt-1 w-full max-w-prose text-sm lg:text-md text-neutral-g100 dark:text-neutral-g50">
                        {{ description }}</p>
                </hgroup>
