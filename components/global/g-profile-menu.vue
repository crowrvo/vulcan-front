    ...
    <div class="relative">
        <button @click="$emit('show-menu')"
            ...
            <div class="relative">
                ...
                <span
                    class="absolute bottom-0 right-0 size-6 flex justify-center items-center ring-1 ring-n0 dark:ring-n900 rounded-full bg-alert-600 text-n0 text-sm lg:text-base">
        ...
        <Transition enter-active-class="transition-opacity duration-150 ease-in" enter-from-class="opacity-0"
            ...
            <menu v-show="showMenu" @focus="$emit('show-menu', true)"
                class="flex flex-col p-1 absolute gap-1 top-full w-full origin-top-right bg-n0 dark:bg-n900 border border-n30 dark:border-n700 divide-y divide-n20 dark:divide-n700 shadow-md rounded-t-none rounded-b-sd border-t-0 focus:outline-0">
                <nav class="w-full">
                    <ul class="w-full">
                        <li class="w-full" v-for="action in actions" :key="action.to">
                            <NuxtLink tabindex="0" @focus="$emit('show-menu', true)"
                                class="w-full capitalize text-base lg:text-md text-n900 dark:text-n0 inline-flex items-center focus:bg-secundary-50 dark:focus:!bg-n700 gap-2 p-3 hover:active:bg-n20 dark:hover:active:bg-n700 focus:outline focus:outline-secundary-300 active:outline-0 rounded"
                                :to="action.to">
                                <g-icon class="!text-n900 dark:!text-n0 text-sm lg:text-md"
                                    :icon="action.icon" />
                                {{ action.name }}
                            </NuxtLink>
                        </li>
                ...
                <button tabindex="0" type="button" @click="" @focus="$emit('show-menu', true)"
                    class="relative text-base lg:text-md w-full capitalize text-n900 dark:text-n0 inline-flex items-center focus:bg-secundary-50 dark:focus:!bg-n700 gap-2 p-3 focus:outline hover:active:bg-n20 dark:hover:active:bg-n700 focus:outline-secundary-300 active:outline-0 rounded">
                ...
                <button tabindex="0" type="button" @click="" @focus="$emit('show-menu', true)"
                    class="capitalize text-base lg:text-md inline-flex items-center text-alert-600 gap-2 p-3 focus:bg-alert-50 hover:active:bg-n20 dark:hover:active:bg-n700 dark:focus:!bg-n700 focus:outline focus:outline-alert-100 dark:focus:outline-alert-900 active:outline-0 rounded">
