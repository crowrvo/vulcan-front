    ...
    <div class="relative">
        <button @click="$emit('show-menu')"
            ...
            <div class="relative">
                ...
                <span
                    class="absolute bottom-0 right-1 size-3 ring-1 ring-neutral-g0 dark:ring-neutral-g900 rounded-full bg-alert-600"></span>
            </div>
            {{ user.name }}
        ...
        <Transition enter-active-class="transition-opacity duration-150 ease-in" enter-from-class="opacity-0"
            ...
            <menu v-show="showMenu" @focus="$emit('show-menu', true)"
                    ...
                    <ul class="w-full">
                        <li class="w-full" v-for="action in actions" :key="action.to">
                            <NuxtLink tabindex="0" @focus="$emit('show-menu', true)"
                                class="w-full capitalize text-neutral-g900 dark:text-neutral-g0 inline-flex items-center focus:bg-secundary-50 dark:focus:!bg-neutral-g700 gap-2 px-3 py-2 text-base lg:text-md hover:active:bg-neutral-g20 dark:hover:active:bg-neutral-g700 focus:outline focus:outline-secundary-300 active:outline-0 rounded"
                                :to="action.to">
                                <g-icon class="!text-neutral-g900 dark:!text-neutral-g0 text-xsm lg:text-md"
                                    :icon="action.icon" />
                                {{ action.name }}
                            </NuxtLink>
                        </li>
                ...
                <button tabindex="0" type="button" @click="" @focus="$emit('show-menu', true)"
                    class="relative w-full capitalize text-neutral-g900 dark:text-neutral-g0 inline-flex items-center focus:bg-secundary-50 dark:focus:!bg-neutral-g700 gap-2 px-3 py-2 text-base lg:text-md focus:outline hover:active:bg-neutral-g20 dark:hover:active:bg-neutral-g700 focus:outline-secundary-300 active:outline-0 rounded">
                    <g-icon class="!text-current text-xsm lg:text-md" icon="uil:bell" />
                    Notificações
                    <span class="absolute top-2 left-3 size-2 rounded bg-alert-600"></span>
                </button>
                <button tabindex="0" type="button" @click="" @focus="$emit('show-menu', true)"
                    class="capitalize inline-flex items-center text-alert-600 gap-2 px-3 py-2 focus:bg-alert-50 hover:active:bg-neutral-g20 dark:hover:active:bg-neutral-g700 dark:focus:!bg-neutral-g700 focus:outline focus:outline-alert-100 dark:focus:outline-alert-900 active:outline-0 rounded">
