...
<template>
    <label @focusout="lostFocus" class="flex flex-col gap-2 text-md lg:text-lg capitalize font-semibold">{{ label }}
        <g-input class="caret-primary-700 text-primary-900 dark:caret-primary-400" v-if="!passwordToggle" :type="type" />
        <div v-else class="self-stretch relative">
            <g-input @keyup.enter="$emit('enter')" class="w-full pr-6 caret-primary-700 text-primary-900 dark:caret-primary-400" :type="inputType" />
