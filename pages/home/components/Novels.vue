    ...
    <section class="flex flex-col gap-3 px-6 py-4 ring-1 ring-n30 dark:ring-0 bg-n20 dark:bg-n800 rounded-2xl">
        <hgroup class="self-stretch flex gap-3">
            <h2 class="text-2xl lg:text-3xl font-secundary text-n900 dark:text-n0 uppercase">
                Novels em <span class="text-secundary-400 dark:text-secundary-300">recém-adicionadas</span></h2>
        </hgroup>
        <div class="flex gap-8">
            <div
                ...
                <div class="flex flex-col gap-2 px-6 pb-6">
                    <NuxtLink
                        class="text-n0 text-center px-6 py-2 rounded-[16px] bg-secundary-400 w-full transition-[background, outline] duration-300 ease-in text-base hover:active:outline hover:active:bg-secundary-500 outline-secundary-400 focus:outline-secundary-400 focus:outline-offset-1"
                        :to="novels[0].novelUrl">
                        Conferir</NuxtLink>
                    <button tabindex="0"
                        class="text-n0 bg-n500 px-6 py-2 rounded-[16px] text-left inline-flex gap-1 w-full select-none ease-in hover:active:outline transition-[background, outline] duration-300 hover:active:bg-n600 outline-n400 focus:outline-n400 focus:outline-offset-1">
            ...
            <div class="flex flex-wrap px-6 gap-6 self-stretch">
                <g-novel-card class="h-min" show-as-variant="portrait" v-for="novel in novels.slice(1, 11)"
                    :key="novel.novelUrl" :title="novel.title" :image="novel.image" :novel-url="novel.novelUrl"
                    :description="novel.description" :rating="novel.rating" :show-chapters="true"
                    :chapters-counter="novel.chaptersCounter" :categories="novel.categories" />
            </div>
        </div>
    </section>
</template>